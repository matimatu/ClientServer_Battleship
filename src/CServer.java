...
class CServerThread extends Thread {
    ...
    public CServerThread(Socket socket1, Socket socket2,
                         CBattleShip battleShip, CBattleShip.CPlayer player1,CBattleShip.CPlayer player2, Semaphore semaphore,boolean myTurn) {
    ...
    public void communicate(){
        try
        {
            ...
            while(!done) //infinite loop to continue communication
            {
                ...
                {
                    outToClient1.println(" (=>leaving the chat...)" + "\n");
                    System.out.println(playerNickname + " requested to leave the chat  :" + messageReceivedFromClient1);
                    break;
                }
                else
                {
                    outToClient2.println(playerNickname + ": " + messageReceivedFromClient1 + "\n");
            ...
            outToClient2.println(" (=>" + playerNickname + " leaved the chat)" + "\n");
        }
        catch (Exception e)
        {
    ...
    private void handleNickname()
    {
        try
        {
            ...
            outToClient1.println("Please enter a nickname before playing");
            playerNickname = inFromClient1.readLine();
            System.out.println("user " + playerNickname + " logged in!");
            outToClient1.println("Welcome to the battleship game " + playerNickname + "!");
        }
        catch(IOException e)
        {
            System.out.println(e.getMessage());
            System.out.println("Server handle nickname error!");
            System.exit(1);
        }
    }
    ...
    private void startBattle(){
        try{
            ...
            if(myTurn)
            {
                ...
                outToClient1.println("Type the coordinates to fire!");
                String coord = inFromClient1.readLine();
                CBattleShip.ShotOutcome shotOutcome = battleShip.fire(player2,coord);
                myTurn = false;
            }
        }
        catch (IOException e)
...
public class CServer {
    ...
    public void start(){
        try{
            ...
            serverThread1 = new CServerThread(socket1,socket2,battleShip,player1,player2,semaphore,true);
            serverThread1.start();
            serverThread2 = new CServerThread(socket2,socket1,battleShip,player2,player1,semaphore,false);
            serverThread2.start();
